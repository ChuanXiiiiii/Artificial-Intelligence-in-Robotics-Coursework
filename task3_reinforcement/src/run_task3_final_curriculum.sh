...
SEED="${1:-42}"
TOTAL_TIMESTEPS="${2:-2000000}"
N_EVAL_EPISODES="${3:-100}"
RUN_SUFFIX="${4:-finalG_curriculum}"

"${PYTHON_BIN}" src/train_agents.py \
	--method ppo \
	--seed "${SEED}" \
	--reward-scheme G \
	--env-config env_final_rewardG.json \
	...
	--curriculum-levelup-threshold 0.55 \
...
echo "Task3 final frozen curriculum run completed: seed=${SEED}, timesteps=${TOTAL_TIMESTEPS}, reward=G"
